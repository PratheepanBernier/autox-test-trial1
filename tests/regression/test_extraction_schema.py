...
def test_carrier_info_happy_path():
    ...
    assert carrier.email == "dispatch@acme.com"
...
def test_shipment_data_full():
    ...
    assert shipment.carrier_instructions == "No partial loads"
...
def test_shipment_data_invalid_field_raises_error():
...
def test_extraction_response_missing_data_raises_error():
...
def test_rate_info_boundary_total_rate_negative():
    rate = RateInfo(total_rate=-100.0, currency="USD")
    assert rate.total_rate == -100.0
    assert rate.currency == "USD"

def test_location_zip_code_edge_cases():
    loc = Location(zip_code="")
    assert loc.zip_code == ""
    loc2 = Location(zip_code=None)
    assert loc2.zip_code is None

def test_commodity_item_weight_and_quantity_edge_cases():
    item = CommodityItem(weight="", quantity="")
    assert item.weight == ""
